defmodule ResearchHarness do
  ...
  alias ResearchHarness.{Runner, Reporter, Utilities}
  ...
  ## Examples
      ...
      {:ok, task_id} = ResearchHarness.run_async(MyExperiment)
      ResearchHarness.status(task_id)
  """
  ...
  defp validate_experiment(experiment_module) do
    ResearchHarness.Experiment.Validator.validate(experiment_module)
  end
  ...
  defp analyze_results(results, config) do
    alias ResearchHarness.Collector
  ...
  defp generate_comparison_matrices(analysis, config) do
    Enum.map(config.metrics, fn metric ->
      {metric, ResearchHarness.Collector.ComparisonMatrix.generate(analysis, metric)}
    end)
    |> Map.new()
  end
